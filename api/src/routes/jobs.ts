...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const queryString =
        "INSERT INTO jobs (id, original_filename) VALUES($1, $2) RETURNING id, status, original_filename";
      ...
      const [jobResult] = await query<
        Pick<Job, "id" | "status" | "original_filename">
...
router.get("/:id", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [jobResult] = await query<
      Pick<Job, "id" | "status" | "created_at" | "updated_at">
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const queryValues = [jobId];
      const [jobResult] = await query<
        Pick<Job, "status" | "original_filename" | "transcription">
      >(queryInput, queryValues);
      // id exists?
      if (!jobResult) {
